...
| `PATCH /agent-servers/{id}/slots/{slot_id}` | Admin | Enable or pause one agent — writes `runner_config.paused` and pushes `config.update`. A claimed run is left to finish | `runner_config.paused`, `agent_slots.desired_state` |
...
| `GET /agent-servers/{id}/slots/idle-reasons` | Org member | US-27.9: why each agent on this host is not working — `working`, `revoked`, `paused`, `no-grants`, `queue-held`, `idle` | Nothing — read-only |
| `GET /agents/idle-reasons?org={org_id}` | Org member | US-35.1: why every agent in the org is not working, keyed by worker **and** principal id. The same `db.worker_idle_reason` the host-scoped route above serves, asked org-wide — the host-scoped one cannot reach an agent that is not on a managed machine. Org scoping is RLS via the caller's own token | Nothing — read-only |
...
The most important table here for an operating agent. `factory_mcp.py` defines **36** tools. Rows
...
| `get_release_changes` | US-21.2: the commit range of the release the claimed run is preparing — commits, changed file paths with status, and the included work items. `path_prefix` narrows the file list; `truncated`/`cursor` page it, and a partial range is reported loudly rather than silently capped | Nothing — read-only | Release run: before writing the notes |
...
| `submit_release_run` | Completes a `release` run: notes titled with the computed version (mismatch refused) stored as a project document; optionally the factory opens the default→UAT promotion PR (US-13.12) | `documents` row; optional PR; `runs.status` → `succeeded`; no environment change | Release run: after assembling the cut |
