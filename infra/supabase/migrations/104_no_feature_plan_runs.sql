-- 104_no_feature_plan_runs (US-11.2): a feature is never planned directly.
--
-- A feature's rail is Draft -> PRD -> Stories; planning happens on the
-- stories the breakdown produces. dispatch_issue never consulted
-- issues.type when choosing the run kind, so a feature that was 'ready',
-- had an approved PRD, and had no children resolved to kind = 'plan' and
-- put a run in the pool that no worker should ever have been offered.
-- Observed 2026-07-20: run 021dc3ef-bc62-4a15-a041-a8223ba7677c against a
-- feature, sitting queued alongside legitimate work.
--
-- dispatch_issue v7 = v6 (061) plus that guard. The v6 check "feature
-- requires an approved PRD before planning" is *replaced*, not kept
-- alongside: it is unreachable once no feature can resolve to a plan run,
-- and leaving it would read as though some feature plan path still exists.
-- The "feature with child stories is not dispatchable" guard stays — it
-- still governs the code path and gives the more specific message.
--
-- Net effect for features: dispatch_issue only ever yields kind = 'code',
-- and only for a feature carrying an approved plan (legacy rows predating
-- this migration). PRD drafting (dispatch_prd_draft, 049) and the story
-- breakdown (dispatch_breakdown, 085) are separate functions and are
-- untouched.
