-- 061_dispatch_test_cases: bundle the issue's test cases into the run's
-- input_context (US-5.7), so the REST context endpoint and MCP
-- get_work_context both carry them from the one source and a coding
-- agent writes tests that match what the manager will verify in UAT.
--
-- dispatch_issue v6: adds 'test_cases' (active cases only — id, title,
-- steps, expected_result) to v_context. Everything else is unchanged
-- from the 053 revision. Runs dispatched before this migration simply
-- lack the key; readers treat missing and empty alike.
