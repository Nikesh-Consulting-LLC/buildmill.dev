...
-- Each per-task file the Default carries (ten on a fresh Default, sixteen on
-- a refreshed one) is the Default's text with one project-specific paragraph
-- in front of it.
...
Work only in `models/marts/`, `tests/`, `docs/conversion-notes/`, and the mart's `schema.yml`. Ask (via clarification) before adding a new seed or macro. Never add credentials or a `.env` file to the repo.$doc01$
  ...
  if v_id is not null then
    v_addenda := jsonb_build_object(
      'prd', $k01_prd$This is a dbt project that replaces a legacy reporting estate. A feature here is a batch of report conversions or a shared reconciliation capability: name the legacy reports (the files under `legacy/`), state the reconciliation rule (each mart matches `legacy/expected/<report>.csv` to the penny; a documented legacy defect is written down as a variance, never matched), and say which planted anomalies in `data/README.md` are in scope. Editing `legacy/` or `data/` is always out of scope.$k01_prd$,
      'plan', $k01_plan$A conversion story's plan names: the legacy source in `legacy/` you read and what its query or formulas actually compute; the mart's grain and measures; which staging and intermediate models it reuses (never recompute NOI from the GL when `int_noi_monthly` exists); the reconciliation test against `legacy_expected.<report>`; and any variance you expect from a documented anomaly. Test-plan cases here look like: 'dbt build passes', 'the mart reconciles to the legacy CSV within 0.005 on its grain', 'the conversion note lists every variance'.$k01_plan$,
      'code', $k01_code$Add the mart under `models/marts/`, its `schema.yml` tests, `tests/reconcile_<report>.sql`, and `docs/conversion-notes/<report>.md`. Do not edit `legacy/`, `data/`, staging or intermediate models. If you can run dbt, run `dbt deps && dbt seed && dbt build` before submitting and report the evidence; a reconciliation that fails on a planted anomaly is excluded row by row with a comment naming the anomaly — never loosened, and never 'fixed' by matching the wrong number.$k01_code$,
      'release', $k01_release$For this dbt project, the notes name each mart added or changed and its reconciliation status (matched, or documented variance); the test cases are 'run dbt build' plus a spot check per mart against `legacy/expected/`.$k01_release$,
      'breakdown', $k01_breakdown$Split by report: one story per legacy report, one mart each, ordered by what staging or intermediate models they need. A defect found in the legacy report itself becomes a bug work item, not a silent fix inside a conversion story.$k01_breakdown$,
      'test', $k01_test$Execute with `dbt deps && dbt seed && dbt build` on the branch; per case, compare the mart to `legacy_expected.<report>` on its grain and read the conversion note for the variances it claims.$k01_test$,
      'deploy', $k01_deploy$This project has no deployment target of its own — a deploy run here should release_work with a note saying so rather than triggering anything.$k01_deploy$,
      'guidelines', $k01_guidelines$The commands are dbt commands (`dbt deps`, `dbt seed`, `dbt build`, `dbt build --select <model>+`) — ground them in `dbt_project.yml`, `packages.yml` and `requirements.txt`. Keep the reconciliation rule and the 'never edit `legacy/` or `data/`' boundary intact in any rewrite.$k01_guidelines$,
      'elaborate', $k01_elaborate$Name the actual `.rdl` or `.xlsx` under `legacy/`, the expected CSV under `legacy/expected/`, and the staging models the mart will read; the acceptance criterion is a grain and a tolerance, not an impression.$k01_elaborate$,
      'wireframe', $k01_wireframe$This project has no user interface. Hand back no_ui_surface naming the mart the story adds.$k01_wireframe$,
      'standalone_plan', $k01_standalone_plan$A standalone story here is one report conversion or one shared test or macro: name the legacy source under `legacy/`, the mart's grain and measures, the staging and intermediate models it reuses, and the reconciliation test against `legacy_expected.<report>`. Test-plan cases: 'dbt build passes', 'the mart reconciles within 0.005 on its grain', 'the conversion note lists every variance'.$k01_standalone_plan$,
      'standalone_code', $k01_standalone_code$The same rules as a feature story: the mart under `models/marts/`, its `schema.yml` tests, `tests/reconcile_<report>.sql` and `docs/conversion-notes/<report>.md`; never touch `legacy/`, `data/`, staging or intermediate models; run `dbt deps && dbt seed && dbt build` before submitting if you can, and report the evidence.$k01_standalone_code$,
      'bug_rca', $k01_bug_rca$A bug here is usually a wrong number. Start from the reconciliation test that fails, then read the legacy source under `legacy/` and the planted anomalies in `data/README.md` before blaming the mart — the legacy report may be the thing that is wrong, and the RCA must say which side is, with the rows that differ.$k01_bug_rca$,
      'bug_fix', $k01_bug_fix$Fix the mart, or the test exclusion, that the RCA named. If the RCA found the legacy report wrong, the fix regenerates `legacy/expected/<report>.csv` from the corrected query with the tool that produces it and removes the exclusion — never both. Run dbt before submitting if you can, and report the evidence.$k01_bug_fix$,
      'chore', $k01_chore$A chore here is repository plumbing — a CI workflow, `dbt docs`, `packages.yml` — never a mart; anything that needs a model is a story. `dbt build` stays green.$k01_chore$,
      'merge', $k01_merge$Before merging, confirm the branch's `dbt build` evidence and that nothing under `legacy/` or `data/` changed — a diff that touches them is sent back, not merged.$k01_merge$
    );
...
Edit only under `CRE Portfolio.SemanticModel/definition/`, `tests/measures_expected.yaml`, and `tools/lint_allowlist.yaml` (removals only). Ask via clarification before changing `expressions.tmdl` or `model.tmdl`. Never modify the `.Report/` folder unless the story is about the report.$doc02$
  ...
  if v_id is not null then
    v_addenda := jsonb_build_object(
      'prd', $k02_prd$This is a Power BI semantic model held as TMDL text. A feature here is a change to the model — a family of measures, a relationship fix, a convention applied across the model: name the measures and relationships, the format strings and description requirements, and the naming convention in `tools/conventions.md`. The report pages and publishing to a workspace are out of scope unless the feature is about them.$k02_prd$,
      'plan', $k02_plan$Name the `.tmdl` files touched (`tables/_Measures.tmdl`, `relationships.tmdl`, a table's partition), the DAX shape (`DIVIDE` over `/`, `CALCULATE` with explicit filters, time intelligence over the marked `Date` table), the format string and one-sentence description, and the `tests/measures_expected.yaml` row. Test-plan cases are lint-shaped: 'the TMDL lint passes', 'the measure has a description and a format string', 'no many-to-many relationship remains'.$k02_plan$,
      'code', $k02_code$Edit TMDL by copying shapes that already exist in the repo — indentation, a fresh `lineageTag` on every new object, measures only in `_Measures`. Add the expected row for any new measure; never grow `tools/lint_allowlist.yaml`. DAX cannot execute in this environment: write it plainly and say so in your hand-back notes. Run `python -m pytest -q` if you can and report the evidence.$k02_code$,
      'release', $k02_release$For this model, the notes list measures added or renamed (old → new) and relationships changed; the test cases are 'open the PBIP in Power BI Desktop, refresh, verify the named measures'.$k02_release$,
      'breakdown', $k02_breakdown$One story per measure family or relationship fix; a bulk rename or reformat across the model is a chore, not a story.$k02_breakdown$,
      'test', $k02_test$Run `python -m pytest -q` on the branch; per case, open the model in Power BI Desktop where one is available and check the measure — otherwise mark the case blocked with 'no Desktop' as the evidence.$k02_test$,
      'deploy', $k02_deploy$This project has no deployment target — a deploy run here should release_work with a note saying so.$k02_deploy$,
      'guidelines', $k02_guidelines$The commands are `python -m pytest -q` and `python tools/tmdl_check.py --explain`; the naming convention is `tools/conventions.md` and stays authoritative. Keep the 'copy TMDL shapes that exist, never invent syntax' rule.$k02_guidelines$,
      'elaborate', $k02_elaborate$Name the actual table and measure names from the TMDL and the convention rule that applies; one acceptance criterion per measure.$k02_elaborate$,
      'wireframe', $k02_wireframe$This project has no user interface unless the story is about the report page. Hand back no_ui_surface naming the measure or relationship the story changes.$k02_wireframe$,
      'standalone_plan', $k02_standalone_plan$A standalone story here is one measure, relationship or convention change: name the `.tmdl` files, the DAX shape, the format string and description, and the `tests/measures_expected.yaml` change. Test-plan cases are lint-shaped.$k02_standalone_plan$,
      'standalone_code', $k02_standalone_code$The same rules as a feature story: copy TMDL shapes that already exist, a fresh `lineageTag` on new objects, measures only in `_Measures`, the expected row added, the allowlist never grown; run `python -m pytest -q` if you can and say that DAX was not executed.$k02_standalone_code$,
      'bug_rca', $k02_bug_rca$A bug here is a measure that returns blank or the wrong number in Desktop. The RCA reads the measure's DAX and the affected table's partition M and names the rows or types that trip it (`data/README.md` lists the planted quirks, such as industrial rents quoted annually). DAX cannot execute here — reason from the model and say what a Desktop check would show.$k02_bug_rca$,
      'bug_fix', $k02_bug_fix$Fix in the measure or the partition's M as the RCA named, update the description and the expected row, run the lint if you can, and say in the hand-back that DAX was not executed.$k02_bug_fix$,
      'chore', $k02_chore$A chore here is a bulk rename, reformat or description sweep across the model — a wide, mechanical diff. List old → new names in the PR body; the lint must pass and the allowlist only shrinks.$k02_chore$,
      'merge', $k02_merge$Before merging, confirm the lint evidence and that nothing under `data/csv/` or `.Report/` changed unless the story is about the report.$k02_merge$
    );
    insert into public.project_template_sections
      (template_id, section_type, section_key, title, content, sort_order)
    select v_id, 'worker_instruction', s.section_key, s.title,
...
Add files under `rentroll/parsers/`, `fixtures/<new format>/`, `expected/<new format>/`, `docs/formats/`; edit `registry.py` to register. Ask via clarification before changing `schema.py`, `base.py` or `normalize.py` — those are shared by every parser. Never modify an existing fixture or golden CSV unless the story is about that format.$doc03$
  ...
  if v_id is not null then
    v_addenda := jsonb_build_object(
      'prd', $k03_prd$This is a parser-per-format package. A feature here is a family of formats or a capability (format detection, an export, a CLI): name the formats, the canonical schema fields affected, and the tie-out rule (parsed totals equal the sheet's own total row). Changing an existing fixture workbook is out of scope.$k03_prd$,
      'plan', $k03_plan$A parser story's plan names: the workbook (an attached document that lands in `fixtures/<format>/` unchanged), the header signature that identifies its sheet, the mapping to the canonical schema by header text — never by column position — how totals and subtotals are tied out, and how an unknown column fails. Test-plan cases: 'the golden CSV matches', 'sum(base_rent_monthly) ties to the sheet total', 'an unknown header raises ParseError'.$k03_plan$,
      'code', $k03_code$Add `rentroll/parsers/<format>.py`, register it in `registry.py`, copy the attached workbook into `fixtures/<format>/` unchanged, produce `expected/<format>/*.csv` with your parser and check the tie against the sheet's total row by hand, and write `docs/formats/<format>.md`. Do not touch `schema.py`, `base.py` or `normalize.py` without a clarification. Run `python -m pytest -q` if you can and report the evidence.$k03_code$,
      'release', $k03_release$For this package, the notes list the formats added; the test cases are 'parse the fixture with `rentroll parse` and compare the totals'.$k03_release$,
      'breakdown', $k03_breakdown$One story per format; detection, the CLI and any export are their own stories.$k03_breakdown$,
      'test', $k03_test$Run `python -m pytest -q` on the branch; per case, run `rentroll parse` on the fixture and check the totals against the sheet.$k03_test$,
      'deploy', $k03_deploy$This project has no deployment target — a deploy run here should release_work with a note saying so.$k03_deploy$,
      'guidelines', $k03_guidelines$The commands are `pip install -e .[dev]`, `python -m pytest -q`, `rentroll parse` and `rentroll validate`; the header-not-position rule and the fixtures-are-immutable rule stay in any rewrite.$k03_guidelines$,
      'elaborate', $k03_elaborate$Name the attached workbook, its sheet or sheets, where the total row sits, and the required columns; the acceptance criterion is a tie-out, not a description.$k03_elaborate$,
      'wireframe', $k03_wireframe$This project has no user interface unless the story adds the CLI or a UI. Hand back no_ui_surface naming the parser the story adds.$k03_wireframe$,
      'standalone_plan', $k03_standalone_plan$A standalone story here is one parser or one capability: name the workbook, the header signature, the mapping by header text, the tie-out and the unknown-column failure. Test-plan cases: 'the golden CSV matches', 'the totals tie', 'an unknown header raises ParseError'.$k03_standalone_plan$,
      'standalone_code', $k03_standalone_code$The same rules as a feature story: the parser, its registration, the fixture copied in unchanged, the golden CSV produced by the parser and tied out by hand, the docs page; `schema.py`, `base.py` and `normalize.py` only after a clarification; run `python -m pytest -q` if you can and report the evidence.$k03_standalone_code$,
      'bug_rca', $k03_bug_rca$A bug here is a parsed number that does not tie, or a row mis-mapped. The RCA opens the fixture under `fixtures/<format>/` and names the sheet, header and row where the parser and the workbook disagree — never the golden CSV first, since the parser produced it.$k03_bug_rca$,
      'bug_fix', $k03_bug_fix$Fix the parser as the RCA named, regenerate the golden CSV with the fixed parser and re-check the tie by hand, and add the failing shape to the tests. Never edit the fixture.$k03_bug_fix$,
      'chore', $k03_chore$A chore here is tooling — pre-commit, `ruff`, CI, packaging — never a parser, a fixture or a golden CSV.$k03_chore$,
      'merge', $k03_merge$Before merging, confirm the pytest evidence and that no existing fixture or golden CSV changed unless the story is about that format.$k03_merge$
    );
    insert into public.project_template_sections
      (template_id, section_type, section_key, title, content, sort_order)
    select v_id, 'worker_instruction', s.section_key, s.title,
...
Work under `waterfall/`, `tests/`, and `sample/`. Ask via clarification before touching `deploy/` or `pyproject.toml` dependencies. Never add or modify anything under `models/` — the oracle workbook is the manager's to change — and never commit a `.env`.$doc04$
  ...
  if v_id is not null then
    v_addenda := jsonb_build_object(
      'prd', $k04_prd$This is a waterfall engine and API whose specification is the Excel model in `models/waterfall_reference.xlsx`: name the scenario tabs the feature must reproduce, the tier kinds (return of capital, preferred return, IRR/MOIC hurdles, catch-up, promote splits) and the API endpoints. Fund-level fees, tax, multi-currency and a UI are out of scope unless the feature names them.$k04_prd$,
      'plan', $k04_plan$Name the tier kinds and structure fields, the scenario tabs that must pass in `test_engine_vs_excel.py`, the invariants (partners' distributions sum to the total; no tier over-allocates), and for API stories the endpoints and the `/healthz` contract. Test-plan cases: 'scenario <tab> matches within 0.005', 'POST /compute returns the workbook's summary cells for the sample deal', 'GET /healthz returns status and the git sha'.$k04_plan$,
      'code', $k04_code$Never edit `models/waterfall_reference.xlsx`, and never mark a scenario xfail — if you believe the workbook is wrong, raise a clarification with the cell reference and your number. Round only in `report.py` and the API responses; keep the `/healthz` shape the deployment health check depends on. Run `python -m pytest -q` if you can and report the evidence.$k04_code$,
      'release', $k04_release$For this service, the notes name scenarios newly passing and endpoints added; the test cases include `GET /healthz` and `GET /version` on the UAT deployment and one `POST /compute` against the sample deal compared to the workbook.$k04_release$,
      'breakdown', $k04_breakdown$Split along the engine: structure model, return of capital and preferred return, IRR hurdles, catch-up, the API, then the American variant and sensitivity — each story names the scenario tabs it makes pass.$k04_breakdown$,
      'test', $k04_test$Run `python -m pytest -q` on the branch; exercise API cases against a local `uvicorn` or the UAT URL in your context.$k04_test$,
      'deploy', $k04_deploy$This project deploys through `deploy/deploy.sh`; the health check is `GET /healthz`, which must return 200 with the git sha, and `GET /version` must report the target's `WATERFALL_ENV`. Verify both before declaring the deployment healthy.$k04_deploy$,
      'guidelines', $k04_guidelines$The commands are `python -m pytest -q` and `uvicorn waterfall.api.main:app --port 8080`; the workbook-is-the-oracle rule and the 'never edit `models/`' boundary stay in any rewrite.$k04_guidelines$,
      'elaborate', $k04_elaborate$Name the scenario tabs and named ranges (`out_lp_irr`, `out_gp_promote`, …) the story must satisfy; one acceptance criterion per scenario or endpoint.$k04_elaborate$,
      'wireframe', $k04_wireframe$This is an engine and an API with no user interface unless the story adds one. Hand back no_ui_surface naming the tier or endpoint the story adds.$k04_wireframe$,
      'standalone_plan', $k04_standalone_plan$A standalone story here is one tier kind, one endpoint or one scenario family: name the scenario tabs it makes pass, the invariants, and for API stories the endpoints and the `/healthz` contract.$k04_standalone_plan$,
      'standalone_code', $k04_standalone_code$The same rules as a feature story: never edit the workbook, never xfail a scenario, round only in `report.py` and the API, keep the `/healthz` shape; run `python -m pytest -q` if you can and report the evidence.$k04_standalone_code$,
      'bug_rca', $k04_bug_rca$A bug here is a scenario that stopped matching, or an endpoint answer that differs from the workbook. The RCA names the tab, the cell, the engine's number and the workbook's, and the tier at which the difference enters — read `models/README.md` for the named ranges before concluding the workbook is wrong.$k04_bug_rca$,
      'bug_fix', $k04_bug_fix$Fix the engine as the RCA named and add the hand-made unit case that would have caught it. If the RCA found the workbook wrong, stop and raise a clarification with the cell reference — the workbook is the manager's to change. Run the tests if you can and report the evidence.$k04_bug_fix$,
      'chore', $k04_chore$A chore here is deploy or service plumbing — `deploy/deploy.sh`, the systemd unit template, dependencies — never the engine; `/healthz` and `/version` keep their shape.$k04_chore$,
      'merge', $k04_merge$Before merging, confirm the pytest evidence and that nothing under `models/` changed; a diff touching `deploy/` was cleared by a clarification.$k04_merge$
    );
    insert into public.project_template_sections
      (template_id, section_type, section_key, title, content, sort_order)
    select v_id, 'worker_instruction', s.section_key, s.title,
...
Add cases under `evals/cases/`, edit `warehouse_qa/schema_notes.md` and `docs/decisions/`; ask via clarification before changing `agent.py`, `guard.py`, `compare.py`, or `evals/run.py`. Never touch `data/` or the committed baseline summary.$doc05$
  ...
  if v_id is not null then
    v_addenda := jsonb_build_object(
      'prd', $k05_prd$This is a text-to-SQL assistant with an eval harness. A feature here is a family of questions or a capability of the assistant (guardrails, the cost report, an MCP server): name the questions as cases and the business definitions to pin. Any warehouse other than the in-repo DuckDB, and any write access, is out of scope.$k05_prd$,
      'plan', $k05_plan$Name the case id and question, the oracle SQL's grain, the tables in `must_use_tables` and `must_not_touch`, and the `schema_notes.md` definition you expect to add — the fix for a wrong answer is a definition, not a code path. Test-plan cases: 'the case passes 3 of 3 runs', 'every previous case still passes', 'the guard rejects a non-SELECT'.$k05_plan$,
      'code', $k05_code$Add `evals/cases/<id>.yaml` with a hand-written oracle SQL, edit `warehouse_qa/schema_notes.md`, and add a `docs/decisions/` page if a definition was pinned. Do not touch `agent.py`, `guard.py`, `compare.py` or `evals/run.py` without a clarification, and never edit an oracle to match the assistant. If `ANTHROPIC_API_KEY` is in your environment, run `python -m pytest -q && python -m evals.run` and report the evidence; if it is not, say so via clarification rather than skipping the eval.$k05_code$,
      'release', $k05_release$For this assistant, the notes list cases added and any definition changes; the eval suite runs on the release itself, so do not describe its result.$k05_release$,
      'breakdown', $k05_breakdown$One story per question (one case each); guardrails, the cost column and the MCP server are separate stories.$k05_breakdown$,
      'test', $k05_test$Run `python -m pytest -q` and, with `ANTHROPIC_API_KEY` in your environment, `python -m evals.run`; per case, the summary table in `reports/summary.md` is the evidence.$k05_test$,
      'deploy', $k05_deploy$This project has no deployment target by default — a deploy run here should release_work with a note saying so.$k05_deploy$,
      'guidelines', $k05_guidelines$The commands are `python -m pytest -q`, `python -m evals.run` and `wqa ask`; the 'definitions, not code paths' rule and the SELECT-only guard stay in any rewrite.$k05_guidelines$,
      'elaborate', $k05_elaborate$Name the case id, the tables and the definition involved; make the acceptance criterion the tolerance and the repetition count.$k05_elaborate$,
      'wireframe', $k05_wireframe$This project has no user interface unless the story adds one. Hand back no_ui_surface naming the case the story adds.$k05_wireframe$,
      'standalone_plan', $k05_standalone_plan$A standalone story here is one case or one capability: name the case id, the oracle's grain, the `must_use_tables` and `must_not_touch` lists, and the schema note you expect to add — the fix is a definition, not a code path.$k05_standalone_plan$,
      'standalone_code', $k05_standalone_code$The same rules as a feature story: the case YAML with a hand-written oracle, the `schema_notes.md` change, a `docs/decisions/` page if a definition was pinned; `agent.py`, `guard.py`, `compare.py` and `evals/run.py` only after a clarification; never edit an oracle to match; run the eval if `ANTHROPIC_API_KEY` is present, otherwise say so.$k05_standalone_code$,
      'bug_rca', $k05_bug_rca$A bug here is a wrong or unstable answer. The RCA runs the case with `--case`, compares the assistant's SQL to the oracle, and names the definition or join the schema notes lack — or the case's oracle, if that is what is wrong; say which. If `ANTHROPIC_API_KEY` is absent, say so via clarification rather than guessing.$k05_bug_rca$,
      'bug_fix', $k05_bug_fix$Fix in `schema_notes.md` — or in the case, if the RCA found the oracle wrong and the manager cleared it — never by special-casing the question in `agent.py`. The case must pass 3 of 3 runs and every other case must still pass.$k05_bug_fix$,
      'chore', $k05_chore$A chore here is harness plumbing — the report format, CI, packaging — never a case or a definition.$k05_chore$,
      'merge', $k05_merge$Before merging, confirm the eval evidence covers every case and that `guard.py` and `compare.py` are unchanged unless a clarification cleared it.$k05_merge$
    );
    insert into public.project_template_sections
      (template_id, section_type, section_key, title, content, sort_order)
    select v_id, 'worker_instruction', s.section_key, s.title,
