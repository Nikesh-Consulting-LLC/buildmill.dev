...
Four separate test suites; there is no single command that runs them all.
...
- **api over HTTP** (us-104.2) — Playwright, in `scripts/testing/`: `cd scripts/testing && npm install && node run-all.mjs`. Deliberately **not** part of the api pytest run, because it needs a *running service* to point at and Essential makes outbound network fail instantly by design. Three layers: a generated auth-boundary test per operation (no credential *and* a forged-but-well-formed one must both be refused — the second is what catches a guard that trusts an unverified JWT), hand-written contract/refusal tests, and opt-in read/mutation tests that stay off behind `ALLOW_MUTATIONS=1` because this API deletes orgs, merges PRs and deploys over SSH. A missing credential skips rather than fails, and the runner prints what was skipped. Its `endpoints.json` catalog is generated by `tools/generate-catalog.py` against a running app — **regenerate it after adding endpoints**, since the auth-boundary layer only covers what the catalog knows.
